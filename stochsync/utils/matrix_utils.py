...
def quats_to_rots(quat: torch.Tensor) -> torch.Tensor:
    """
...
def rot_to_quat(rotation_matrix: torch.Tensor, eps: float = 1e-6) -> torch.Tensor:
    """
...
def rots_to_quats(rotation_matrix: torch.Tensor, eps: float = 1e-6) -> torch.Tensor:
    """
    ...
    Returns:
        A tensor representing the corresponding batch of quaternions (w, x, y, z).
    """
...
def make_quat(axis: torch.Tensor, angle: float) -> torch.Tensor:
    """
...
def multiply_quats(q1: torch.Tensor, q2: torch.Tensor) -> torch.Tensor:
    """
