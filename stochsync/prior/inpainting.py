...
class InpaintingPrior(StableDiffusionPrior):
    def __init__(self, cfg):
        ...
        if not (
            (self.cfg.width == self.cfg.height == 512)
            or (self.cfg.width == self.cfg.height == 64)
        ):
    ...
    def prepare_cond(
        ...
        mask_condition = sm.prior.pipeline.mask_processor.preprocess(
            mask, height=mask.shape[-2], width=mask.shape[-1]
        )
    ...
    def predict(
        ...
        self.prepare_cond(
            camera, text_prompt, negative_prompt, mask_image_latents, mask
        )
        guidance_scale = (
