...
def shade(
    ...
    # kd = material['kd']
    # ks = material['ks']
    # kd = dr.texture(material['kd'].contiguous(), gb_texc, gb_texc_deriv)
    # ks = dr.texture(material['ks'].contiguous(), gb_texc, gb_texc_deriv)
    ...
    if bsdf == "pbr":
        ...
        shaded_col = torch.sum(gb_normal * view_rays, dim=-1, keepdim=True).repeat(
            1, 1, 1, 3
        )
    elif bsdf == "faceid":
        # face id
        shaded_col = rast_out_s[..., 3:4].repeat(1, 1, 1, 3)
    else:
...
def render_layer(
    ...
    gb_mask, _ = interpolate(
        torch.ones_like(mesh.v_pos[None, ..., :1]), rast_out_s, mesh.t_pos_idx.int()
    )
