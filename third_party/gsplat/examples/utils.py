...
def print_warning(text: str) -> None:
    print(color.yellow + '[Warning] ' + text + color.end)

def print_info(text: str) -> None:
    print(color.blue + '[Info] ' + text + color.end)

def print_error(text: str) -> None:
    print(color.red + '[Error] ' + text + color.end)

def convert_to_ply(data, ply_path):
    #data = torch.load(checkpoint_path)

    xyz = data['means3d'].cpu().numpy()
    opacities = data['opacities'].cpu().numpy()
    scales = data['scales'].cpu().numpy()
    quats = data['quats'].cpu().numpy()

    N = xyz.shape[0]
    features_dc = data['sh0'].view(N, -1).cpu().numpy()
    features_rest = data['shN'].view(N, -1).cpu().numpy()

    feature_dc_columns = [f"f_dc_{i}" for i in range(features_dc.shape[1])]
    feature_rest_columns = [f"f_rest_{i}" for i in range(features_rest.shape[1])]

    vertex_data = np.array(
        [
            tuple(
                np.concatenate(
                    (xyz[i], scales[i], quats[i], [opacities[i]], features_dc[i], features_rest[i])
                )
            )
        ...
        dtype=[
            ...
            ("rot_3", "f4"),
            ("opacity", "f4"),
            *[(name, "f4") for name in feature_dc_columns],
            *[(name, "f4") for name in feature_rest_columns],
        ],
    )
    print(xyz.shape, scales.shape, quats.shape, opacities.shape, features_dc.shape, features_rest.shape)
