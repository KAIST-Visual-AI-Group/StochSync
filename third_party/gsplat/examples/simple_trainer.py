...
from datasets.blender import Dataset, Parser
...
from utils import convert_to_ply, save_tensor
...
class Config:
    ...
    # Initial number of GSs. Ignored if using sfm
    max_num_pts: int = 300_000
    # Initial extent of GSs as a multiple of the camera extent. Ignored if using sfm
    init_extent: float = 0.2
...
class Runner:
    ...
    def __init__(self, cfg: Config) -> None:
        ...
        self.intermediate_dir = f"{cfg.result_dir}/intermediate"
        ...
        self.running_stats = {
            ...
            "weights": torch.zeros(n_gauss, device=self.device, dtype=torch.float32),
        }
    ...
    def train(self):
        ...
        for step in pbar:
            ...
            gt_alpha = None
            if cfg.random_bkgd and "alpha" in data:
            ...
            if cfg.random_bkgd and gt_alpha is not None:
                bkgd = torch.rand(1, 3, device=device)
                colors = colors + bkgd * (1.0 - alphas)
                pixels = pixels + bkgd * (1.0 - gt_alpha)
            ...
            if cfg.tb_every > 0 and step % cfg.tb_every == 0:
                ...
                canvas = canvas.squeeze(0).transpose(2, 0, 1)
                save_tensor(torch.from_numpy(canvas), f"{self.intermediate_dir}/step{step:04d}.png")
            ...
            if step < cfg.refine_stop_iter:
                ...
                if step > cfg.refine_start_iter and step % cfg.refine_every == 0:
                    ...
                    sample_size = min(
                        len(self.splats['means3d']), cfg.max_num_pts, prob.nonzero().size(0)
                    )
                    indices = np.random.choice(
                        len(self.splats['means3d']), sample_size, replace=False, p=prob.cpu().numpy()
                    )
                    mask = torch.zeros(len(self.splats['means3d']), device=device, dtype=torch.bool)
                    mask[indices] = True
                    self.refine_keep(mask)
                    print(
                        f"Step {step}: Resampled GSs. " f"Now having {len(self.splats['means3d'])} GSs."
                    )
        ...
        convert_to_ply(self.splats.state_dict(), f"{cfg.result_dir}/final_model.ply")
