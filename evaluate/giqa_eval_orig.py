...
class GIQAEvaluator:
    def __init__(
        ...
        self.gpu_id = gpu_id
        self.method = method
        self.exist_ref_act_path = exist_ref_act_path
    ...
    def __call__(
        self,
        ref_image_dir: Optional[Union[str, os.PathLike]] = None,
        fake_image_dir: Optional[Union[str, os.PathLike]] = None,
        ...
        if fake_image_dir is None:
            fake_image_dir = self.fake_image_dir
        if save_dir is None:
        ...
        if self.exist_ref_act_path is None:
            ref_act_path = Path(save_dir) / f"ref_act_method_{self.method}.pkl"
            self.extract_features(ref_image_dir, ref_act_path, gpu_id)
            print(f"[*] Computed reference image features")
        else:
            exist_ref_act_path = Path(self.exist_ref_act_path)
            assert (
                exist_ref_act_path.exists()
            ), f"Cannot load existing refence features from {exist_ref_act_path}"
            ref_act_path = exist_ref_act_path
            print(f"[*] Skipped computing reference image features")

        now = get_current_time()
        output_file_path = Path(save_dir) / f"all_knn_scores_method_{self.method}_{now}.txt"
